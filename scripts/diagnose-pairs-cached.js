...
import { buildBenchmarkModelConfig } from '../src/benchmark-defaults.js';
import { dot } from '../src/math.js';

const suite = loadBenchmarkSuite('benchmarks/react-component-duplicates-v1/manifest.json');
const cachePath = '.cache/duplicalis/benchmarks/react-component-duplicates-v1/embeddings.json';
const baseConfig = {
  root: suite.root,
  ...
  cleanProbability: 0,
  remote: {
    url: process.env.API_URL || 'https://openrouter.ai/api/v1/embeddings',
...
const files = await findSourceFiles(baseConfig);
const parsed = loadComponentsWithCache(files, baseConfig);

const models = [
  { id: 'openai/text-embedding-3-small', kind: 'remote', label: 'OpenAI 3 small' },
  { id: 'openai/text-embedding-3-large', kind: 'remote', label: 'OpenAI 3 large' },
];

const results = {};
for (const spec of models) {
  const modelConfig = buildBenchmarkModelConfig(baseConfig, spec);
  const backend = await createEmbeddingBackend(modelConfig);
  const embedded = await embedComponents(parsed.components, backend, modelConfig);
  const byId = new Map(embedded.entries.map((e) => [e.component.id, e]));

  console.error(`${spec.label}: ${embedded.cacheStats.hits} hits, ${embedded.cacheStats.misses} misses`);
  ...
  for (const key of suite.positivePairs) {
    ...
    const eb = byId.get(b);
    if (ea && eb) {
      posScores.push({ a: a.split('#')[1], b: b.split('#')[1], score: dot(ea.vector, eb.vector) });
    }
  }
  ...
  for (const pair of suite.hardNegativePairs) {
    ...
    if (ea && eb) {
      hnScores.push({ a: pair.a.split('#')[1], b: pair.b.split('#')[1], score: dot(ea.vector, eb.vector) });
    }
  }
  hnScores.sort((x, y) => y.score - x.score);

  results[spec.id] = { posScores, hnScores };
}
...
console.log('Pair'.padEnd(50) + 'Small'.padStart(8) + 'Large'.padStart(8) + 'Delta'.padStart(8));
for (const sp of small.posScores) {
  ...
  console.log(
    `${sp.a} / ${sp.b}`.padEnd(50) +
      ...
      (ls - sp.score).toFixed(4).padStart(8)
  );
}

console.log('\n=== HARD NEGATIVE PAIRS (sorted by small score desc) ===');
console.log('Pair'.padEnd(55) + 'Small'.padStart(8) + 'Large'.padStart(8) + 'Delta'.padStart(8));
...
console.log('\n=== LARGE: HN pairs above min-positive ===');
for (const lp of large.hnScores.filter((p) => p.score >= largeMinPos)) {
  console.log(`  ${lp.a} / ${lp.b}: ${lp.score.toFixed(4)}`);
}
